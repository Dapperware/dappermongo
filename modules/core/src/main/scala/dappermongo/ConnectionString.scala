...
class ConnectionString private (protected val inner: JConnectionString) extends ConnectionStringVersionSpecific {
  private[dappermongo] def asJava: JConnectionString = inner
...
object ConnectionString {
  ...
  private[dappermongo] def unsafe(uri: String): ConnectionString = new ConnectionString(new JConnectionString(uri))
